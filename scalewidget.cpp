...
QString str = QString("0") ;
    painter.drawText(-(painter.fontMetrics().boundingRect(str).width()>>1)+width()/2-w/2,24,str) ;
 str = "("+settings.value("Crop/UnitName","").toString()+")" ;
        painter.drawText(-(painter.fontMetrics().boundingRect(str).width()>>1)+width()*.875,16,str) ;
for (int i = 0 ; i < 5 ; i++)
{
    if (i%2)
...
QString str = QString("%1").arg(roundmm*(i+1)/5.0,0,'g',3);
    painter.drawText((i+1)*w/5-(painter.fontMetrics().boundingRect(str).width()>>1)+width()/2-w/2,24,str) ;
}
}
