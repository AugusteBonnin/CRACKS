...
//    ui->label_6->setFont(font);
    ...
    DoubleRangedHistoResult * dhr =  new DoubleRangedHistoResult(this,tr("Différence d'angle aux places"),mainWindow->histoDoubleData[0],1,0,180) ;
    histoResults << dhr ;
    dhr->setFixedSize(QSize(800,400));

    DoubleHistoResult * closeness =  new DoubleHistoResult(this,tr("Accessibilité des voies"),mainWindow->histoDoubleData[1],2) ;
    histoResults << closeness ;
    closeness->setFixedSize(QSize(800,400));

    IntHistoResult * degree =  new IntHistoResult(this,tr("Degré des voies"),mainWindow->histoIntData[1],3) ;
    histoResults << degree ;
    degree->setFixedSize(QSize(800,400));

    DoubleHistoResult * ortho =  new DoubleHistoResult(this,tr("Orthogonalité des voies"),mainWindow->histoDoubleData[2],4) ;
    histoResults << ortho ;
    ortho->setFixedSize(QSize(800,400));

    DoubleHistoResult * spacing =  new DoubleHistoResult(this,tr("Espacement des voies"),mainWindow->histoDoubleData[3],5) ;
    histoResults << spacing ;
    spacing->setFixedSize(QSize(800,400));

    DoubleHistoResult * length_roads =  new DoubleHistoResult(this,tr("Longeur odométrique des voies"),mainWindow->histoDoubleData[4],6) ;
    histoResults << length_roads ;
    length_roads->setFixedSize(QSize(800,400));

    DoubleHistoResult * length_edges =  new DoubleHistoResult(this,tr("Longeur odométrique des arcs"),mainWindow->histoDoubleData[5],7) ;
    histoResults << length_edges ;
    length_edges->setFixedSize(QSize(800,400));

    IntHistoResult * length_topo =  new IntHistoResult(this,tr("Longeur topologique des voies"),mainWindow->histoIntData[2],8) ;
