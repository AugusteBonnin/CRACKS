            ...
            for (int j = 0 ; j < skel_children[i].count() ; j++)
            {
                ...
                if (!frame.contains(p2))
                {
                    ...
                    for (int k = 0 ; k < 4 ; k++)
                        ...
                        {
                            //p1 is inside frame
                            skel_children[i2].clear() ;
                            skel_children[i][j] = skel_points.count() ;
            ...
            for (int j = 0 ; j < skel_children[i].count() ; j++)
            {
                ...
                if (frame.contains(p2))
                {
                    ...
                    for (int k = 0 ; k < 4 ; k++)
                        ...
                        {
                            //p2 is inside frame
                            skel_children[i].clear() ;
                            skel_children[i2][skel_children[i2].indexOf(i)] = skel_points.count() ;
...
cdt.clear();
    ...
    QRectF frame(QPointF(0,0),QPointF(mainWindow->openedImage->width(),mainWindow->openedImage->height())) ;
    ...
    {
        if (frame.contains(QPointF(it->point().x(),it->point().y())))
        {
                ...
                if (next->info().in_domain())
                {
                    if (current->info().in_domain())
                    {
                        ...
                        else
                        {
                            ...
                            skel_distance.append(QLineF(A,center).length()) ;
                            skel_points <<  center ;
                    ...
                    else //current face not in domain
                    {
                        int middle_idx ;
                        ...
                        else
                        {
                            ...
                            QLineF AB(A,B);
                            QPointF middle = AB.pointAt(.5) ;
                            middle_idx = skel_points.count() ;
                            middlesIdx[pair] = skel_points.count() ;
                            skel_points << middle ;
                            skel_distance.append(QLineF(A,middle).length()) ;
                            skel_children.append( QVector<unsigned int>()) ;
                        }
                        ...
                        skel_children[middle_idx].append( center_t2_idx );
                    }
                }
                else // next face not in domain
                {
                    if (current->info().in_domain())
                    {
                        int middle_idx ;
        ...
        for (int i = 0 ; i < skel_children.count() ; i++)
        {
            ...
            //if (v.count()==1)
            {
                ...
                if (skel_distance[current]>=length*settings.value("Skel/Prune",2.0).toDouble())
                {
                    ...
                    if (skel_children[str[str.count()-1]].indexOf(str[str.count()-2])>-1)
                    skel_children[str[str.count()-1]].remove(skel_children[str[str.count()-1]].indexOf(str[str.count()-2]));

                    for (int j = 1 ; j < str.count() - 1 ; j++)
                        skel_children[str[j]].clear();

                    if (skel_children[str[0]].indexOf(str[1])>-1)
                    skel_children[str[0]].remove(skel_children[str[0]].indexOf(str[1]));
                }
            }
